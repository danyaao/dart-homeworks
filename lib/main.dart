...
void main() {
  ...
  for (final articleAsString in articlesStrings) {
    List<String> productAsList = articleAsString.split(',');
  ...
  applyFilter(
    products: products,
    filter: CategoryFilter(filteredCategory: 'хлеб'),
  );
  ...
  applyFilter(
    products: products,
    filter: PriceFilter(filteredPrice: 500),
  );
  ...
  applyFilter(
    products: products,
    filter: QuantityFilter(filteredQuantity: 10),
  );
}
